...
namespace Your_Lib_Test_Namespace {
    ...
    class Your_Lib_Name_Json final : public jtu::JsonBase{
    public:
    ...
    class Your_Lib_Name_Str final : public jtu::StringBase{
    public:
        // 存放你库 最常用的 字符串类型
        Your_String_Type str;
        // 重写虚函数，将你的库的字符串转换成标准字符串，此步骤不计时间。
        std::string get_string() override { return str.to_std_string(); }
    };
    ...
    */
    // 例：
    static void get_element_num(const Your_Json_Type& json, jtu::JsonCounter& counter){
        switch (json.type()){
            case JSON::OBJECT: {
            ...
            case JSON::NUMBER:
                ...
                break;
            case JSON::BOOLEN:
                if(IS_TRUE(json)) counter.m_true_num += 1;
    ...
    */
    ...
    class Your_Lib_Name_Test final : public jtu::TestBase{


        // 1. 重写 反序列化函数   样例如下
        std::shared_ptr<jtu::JsonBase> deserialize(const std::string & str) override{
            // 定义 第1步 定义的 JSON子类指针
            const auto json_ptr = std::make_shared<Your_Lib_Name_Json>();
            try{
                // 用 JSON子类指针 内部的 JSON数据对象 反序列化字符串
                json_ptr->json = Your_Parse_Json_Function(str);
            }catch(...){
        ...
        std::shared_ptr<jtu::StringBase> serialize(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            const auto str_ptr = std::make_shared<Your_Lib_Name_Str>();
            try{
                // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
                const auto json_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
        ...
        void add_child_to_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<jtu::JsonBase> value) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
                const auto value_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(value);
        ...
        std::shared_ptr<jtu::JsonBase> get_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string & str) override{
            ...
            const auto json_ptr = std::make_shared<Your_Lib_Name_Json>();
            try{
                const auto old_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
    ...
    M_REGISTER_CLASS("Your_Lib_Name", Your_Lib_Name_Test)

...
class Your_Lib_Name_Json : public JsonBase{
public:
...
class Your_Lib_Name_Str : public StringBase{
public:
    // 存放你库 最常用的 字符串类型
    Your_String_Type str;
    // 重写虚函数，将你的库的字符串转换成标准字符串，此步骤不计时间。
    std::string getString() override{ return str.to_std_string(); }
};
...
*/ 
// 例：
static void get_element_num(const Your_Json_Type& json, JsonCounter& counter){
    switch (json.type())
    {
    case JSON::OBJECT:
...
class Your_Lib_Name_Test : public TestBase{
    

    // 1. 重写 反序列化函数   样例如下
    std::shared_ptr<JsonBase> deserialize(const std::string & str) override{
        // 定义 第1步 定义的 JSON子类指针
        auto json_ptr = std::make_shared<Your_Lib_Name_Json>();
        try{
    ...
    std::shared_ptr<StringBase> serialize(std::shared_ptr<JsonBase> json_base_ptr) override{
        ...
        auto str_ptr = std::make_shared<Your_Lib_Name_Str>();
        try{
            // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
            auto json_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
    ...
    void add_child_to_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<JsonBase> value) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
            auto value_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(value);
    ...
    std::shared_ptr<JsonBase> get_child_from_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string & str) override{
        ...
        auto json_ptr = std::make_shared<Your_Lib_Name_Json>();
        try{
            auto old_ptr = std::dynamic_pointer_cast<Your_Lib_Name_Json>(json_base_ptr);
...
REGISTER_CLASS("Your_Lib_Name", Your_Lib_Name_Test)

...
class JsoncppObj : public JsonBase{
public:
...
class JsoncppStr : public StringBase{
public:
    // 存放你库 最常用的 字符串类型
    std::string str;
...
*/ 
// 例：
static void get_element_num(const Json::Value& json, JsonCounter& counter){
    switch (json.type())
    {
...
class JsoncppTest : public TestBase{
    ...
    std::shared_ptr<JsonBase> deserialize(const std::string & str) override{
        // 定义 第1步 定义的 JSON子类指针
        auto json_ptr = std::make_shared<JsoncppObj>();
        try{
            Json::Reader reader;
            // 用 JSON子类指针 内部的 JSON数据对象 反序列化字符串
            if(!reader.parse(str,json_ptr->json)) throw FailException {}; 
        }catch(...){ 
    ...
    std::shared_ptr<StringBase> serialize(std::shared_ptr<JsonBase> json_base_ptr) override{
        ...
        auto str_ptr = std::make_shared<JsoncppStr>();
        try{
            Json::FastWriter writer;
            // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            // 将 JSON子类指针 内部的 JSON数据对象 序列化成字符串，并赋值给 String子类指针 内部的字符串变量
            str_ptr->str = writer.write(json_ptr->json);
        }catch(...){ 
            throw FailException {}; 
        }
    ...
    void add_child_to_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<JsonBase> value) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            auto value_ptr = std::dynamic_pointer_cast<JsoncppObj>(value);
    ...
    void add_child_to_array(std::shared_ptr<JsonBase> json_base_ptr, const size_t& index,  std::shared_ptr<JsonBase> value) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            auto value_ptr = std::dynamic_pointer_cast<JsoncppObj>(value);
            if(!json_ptr->json.isArray() || index > json_ptr->json.size()) throw FailException {}; 
    ...
    void delete_child_from_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string& key) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!json_ptr->json.isObject() || !json_ptr->json.isMember(key)) throw FailException {}; 
    ...
    void delete_child_from_array(std::shared_ptr<JsonBase> json_base_ptr, const size_t& index) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!json_ptr->json.isArray() || index >= json_ptr->json.size()) throw FailException {}; 
            json_ptr->json.removeIndex((int)index , nullptr);
        }catch(...){ 
            throw FailException {}; 
        }
    }
    ...
    std::shared_ptr<JsonBase> get_child_from_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string & str) override{
        ...
        auto json_ptr = std::make_shared<JsoncppObj>();
        try{
            auto old_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!old_ptr->json.isObject() || !old_ptr->json.isMember(str)) throw FailException {}; 
    ...
    std::shared_ptr<JsonBase> get_child_from_array(std::shared_ptr<JsonBase> json_base_ptr, const size_t& index) override{
        ...
        try{
            auto old_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!old_ptr->json.isArray() || index >= old_ptr->json.size()) throw FailException {}; 
            json_ptr->json = old_ptr->json[(int)index];
            return std::static_pointer_cast<JsonBase>(json_ptr);
        }catch(...){ 
            throw FailException {}; 
        }
    ...
    double get_value_as_double(std::shared_ptr<JsonBase> json_base_ptr) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!json_ptr->json.isDouble() && !json_ptr->json.isIntegral()) throw FailException {}; 
    ...
    long long get_value_as_int64(std::shared_ptr<JsonBase> json_base_ptr) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<JsoncppObj>(json_base_ptr);
            if(!json_ptr->json.isIntegral()) throw FailException {}; 
...
REGISTER_CLASS("Jsoncpp", JsoncppTest)

...
class NlohmannJsonObj : public JsonBase{
public:
...
class NlohmannJsonStr : public StringBase{
public:
    // 存放你库 最常用的 字符串类型
    std::string str;
...
*/ 
// 例：
static void get_element_num(const nlohmann::json& json, JsonCounter& counter){
    switch (json.type())
    {
...
class NlohmannJsonTest : public TestBase{
    

    // 1. 重写 反序列化函数   样例如下
    std::shared_ptr<JsonBase> deserialize(const std::string & str) override{
        // 定义 第1步 定义的 JSON子类指针
        auto json_ptr = std::make_shared<NlohmannJsonObj>();
        try{
    ...
    std::shared_ptr<StringBase> serialize(std::shared_ptr<JsonBase> json_base_ptr) override{
        ...
        auto str_ptr = std::make_shared<NlohmannJsonStr>();
        try{
            // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
            auto json_ptr = std::dynamic_pointer_cast<NlohmannJsonObj>(json_base_ptr);
    ...
    void add_child_to_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<JsonBase> value) override{
        ...
        try{
            auto json_ptr = std::dynamic_pointer_cast<NlohmannJsonObj>(json_base_ptr);
            auto value_ptr = std::dynamic_pointer_cast<NlohmannJsonObj>(value);
    ...
    std::shared_ptr<JsonBase> get_child_from_object(std::shared_ptr<JsonBase> json_base_ptr, const std::string & str) override{
        ...
        auto json_ptr = std::make_shared<NlohmannJsonObj>();
        try{
            auto old_ptr = std::dynamic_pointer_cast<NlohmannJsonObj>(json_base_ptr);
...
REGISTER_CLASS("nlohmann-json", NlohmannJsonTest)

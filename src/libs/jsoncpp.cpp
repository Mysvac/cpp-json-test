...
namespace JsoncppTestNamespace {
    /* ----------------------------- 1 ----------------------------- */
    //////////////////////////////////////////////////////////////////
    /////// 重写2个类型

    //// 1.1. 重写JsonBase类，内部存放你的库的，JSON类型。类名任意。
    class JsoncppObj  final : public jtu::JsonBase{
    public:
    ...
    class JsoncppStr final : public jtu::StringBase{
    public:
        // 存放你库 最常用的 字符串类型
        std::string str;
    ...
    static void get_element_num(const Json::Value& json, jtu::JsonCounter& counter){
        switch (json.type()){
            ...
            case Json::booleanValue:
                if(json.asBool()) counter.m_true_num += 1;
    ...
    */
    ...
    class JsoncppTest final : public jtu::TestBase{
        Json::Reader reader{};
        ...
        std::shared_ptr<jtu::JsonBase> deserialize(const std::string & str) override{
            // 定义 第1步 定义的 JSON子类指针
            const auto json_ptr = std::make_shared<JsoncppObj >();
            try{
                // 用 JSON子类指针 内部的 JSON数据对象 反序列化字符串
                if(!reader.parse(str,json_ptr->json)) throw jtu::FailException {};
            }catch(...){
        ...
        std::shared_ptr<jtu::StringBase> serialize(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            const auto str_ptr = std::make_shared<JsoncppStr>();
            try{
                // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                // 将 JSON子类指针 内部的 JSON数据对象 序列化成字符串，并赋值给 String子类指针 内部的字符串变量
                str_ptr->str = fast_writer.write(json_ptr->json);
            }catch(...){
                throw jtu::FailException {};
            }
        ...
        std::shared_ptr<jtu::StringBase> serialize_pretty(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                str_ptr->str = style_writer.write(json_ptr->json);
            }catch(...){
                throw jtu::FailException {};
            }
            return std::static_pointer_cast<jtu::StringBase>(str_ptr);
        ...
        void add_child_to_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<jtu::JsonBase> value) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                const auto value_ptr = std::dynamic_pointer_cast<JsoncppObj >(value);
        ...
        void delete_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                json_ptr->json.removeMember(key);
            }catch(...){
                throw jtu::FailException {};
            }
        }
        ...
        void delete_child_from_array(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::size_t& index) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                json_ptr->json.removeIndex(static_cast<int>(index), nullptr);
            }catch(...){
                throw jtu::FailException {};
            }
        }
        ...
        std::shared_ptr<jtu::JsonBase> get_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string & str) override{
            ...
            const auto json_ptr = std::make_shared<JsoncppObj >();
            try{
                const auto old_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                if(!old_ptr->json.isObject() || !old_ptr->json.isMember(str)) throw jtu::FailException {};
                json_ptr->json = old_ptr->json[str];
                return std::static_pointer_cast<jtu::JsonBase>(json_ptr);
            }catch(...){
                throw jtu::FailException {};
            }
        ...
        std::shared_ptr<jtu::JsonBase> get_child_from_array(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::size_t& index) override{
            ...
            try{
                const auto old_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                if(!old_ptr->json.isArray() || index >= old_ptr->json.size()) throw jtu::FailException {};
                json_ptr->json = old_ptr->json[ static_cast<int>(index) ];
                return std::static_pointer_cast<jtu::JsonBase>(json_ptr);
            }catch(...){
                throw jtu::FailException {};
            }
        ...
        double get_value_as_double(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                if(!json_ptr->json.isDouble() && !json_ptr->json.isIntegral()) throw jtu::FailException {};
                return json_ptr->json.asDouble();
            }catch(...){
                throw jtu::FailException {};
            }
            return 0;
        ...
        long long get_value_as_int64(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<JsoncppObj >(json_base_ptr);
                if(!json_ptr->json.isIntegral()) throw jtu::FailException {};
                return json_ptr->json.asInt64();
            }catch(...){
                throw jtu::FailException {};
            }
            return 0;
        ...
        std::string get_value_as_string(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
            ...
            try{
                ...
                return json_ptr->json.asCString();
            }catch(...){
                throw jtu::FailException {};
            }
            return "";
    ...
    M_REGISTER_CLASS("jsoncpp", JsoncppTest)

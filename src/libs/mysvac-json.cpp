...
class MysvacJsonlibObj final : public jtu::JsonBase{
public:
...
class MysvacJsonlibStr final : public jtu::StringBase{
public:
    // 存放你库 最常用的 字符串类型
    std::string str;
...
*/
// 例：
static void get_element_num(const Jsonlib::JsonValue& json, jtu::JsonCounter& counter){
    switch (json.type()){
        ...
        case Jsonlib::JsonType::BOOL:
            if (json.as_bool()) ++counter.m_true_num;
            else ++counter.m_false_num;
            break;
...
class MysvacJsonlibTest final : public jtu::TestBase{


    // 1. 重写 反序列化函数   样例如下
    std::shared_ptr<jtu::JsonBase> deserialize(const std::string & str) override{
        // 定义 第1步 定义的 JSON子类指针
        const auto json_ptr = std::make_shared<MysvacJsonlibObj>();
        try{
    ...
    std::shared_ptr<jtu::StringBase> serialize(std::shared_ptr<jtu::JsonBase> json_base_ptr) override{
        ...
        const auto str_ptr = std::make_shared<MysvacJsonlibStr>();
        try{
            // 将输入的 JSON父类指针 转换成 第1步定义的 JSON子类指针
            const auto json_ptr = std::dynamic_pointer_cast<MysvacJsonlibObj>(json_base_ptr);
    ...
    void add_child_to_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<jtu::JsonBase> value) override{
        ...
        try{
            const auto json_ptr = std::dynamic_pointer_cast<MysvacJsonlibObj>(json_base_ptr);
            const auto value_ptr = std::dynamic_pointer_cast<MysvacJsonlibObj>(value);
    ...
    std::shared_ptr<jtu::JsonBase> get_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string & str) override{
        ...
        const auto json_ptr = std::make_shared<MysvacJsonlibObj>();
        try{
            const auto old_ptr = std::dynamic_pointer_cast<MysvacJsonlibObj>(json_base_ptr);
...
M_REGISTER_CLASS("mysvac-jsonlib", MysvacJsonlibTest)

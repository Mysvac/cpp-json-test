...
namespace VctToolsJsonTestNamespace {
    /* ----------------------------- 1 ----------------------------- */
    //////////////////////////////////////////////////////////////////
    /////// 重写2个类型

    //// 1.1. 重写JsonBase类，内部存放你的库的，JSON类型。类名任意。
    class VctToolsJson final : public jtu::JsonBase{
    public:
        // 存放你的库中，操作JSON需要使用的类对象
        Json json;
    };

    //// 1.2. 重写StringBase类，内部存放你的库的字符串类。
    class VctToolsString final : public jtu::StringBase{
    public:
        // 存放你库 最常用的 字符串类型
        std::string str;
    ...
    */
    ...
    static void get_element_num(const Json& json, jtu::JsonCounter& counter){
        switch (json.type()){
            ...
            case json::Type::eString:
                counter.m_string_num += 1;
                break;
            case json::Type::eNumber:
                counter.m_number_num += 1;
                break;
            case json::Type::eBool:
                if (json.to<bool>()) counter.m_true_num += 1;
                else  counter.m_false_num += 1;
                break;
            case json::Type::eNull:
    ...
    */
    ...
    class VctToolsJsonTest final : public jtu::TestBase{
        ...
        void add_child_to_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key, std::shared_ptr<jtu::JsonBase> value) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<VctToolsJson>(json_base_ptr);
                const auto value_ptr = std::dynamic_pointer_cast<VctToolsJson>(value);
        ...
        void add_child_to_array(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::size_t& index,  std::shared_ptr<jtu::JsonBase> value) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<VctToolsJson>(json_base_ptr);
                const auto value_ptr = std::dynamic_pointer_cast<VctToolsJson>(value);
                json_ptr->json.arr().insert(json_ptr->json.arr().cbegin()+index, value_ptr->json);
            }catch(...){
                throw jtu::FailException {};
            }
        }
        ...
        void delete_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string& key) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<VctToolsJson>(json_base_ptr);
                if (!json_ptr->json.obj().erase(key)) throw jtu::FailException {};
            }catch(...){
                throw jtu::FailException {};
            }
        }
        ...
        void delete_child_from_array(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::size_t& index) override{
            ...
            try{
                const auto json_ptr = std::dynamic_pointer_cast<VctToolsJson>(json_base_ptr);
                json_ptr->json.arr().erase(json_ptr->json.arr().begin()+index);
            }catch(...){
                throw jtu::FailException {};
            }
        }
        ...
        std::shared_ptr<jtu::JsonBase> get_child_from_object(std::shared_ptr<jtu::JsonBase> json_base_ptr, const std::string & str) override{
            ...
            const auto json_ptr = std::make_shared<VctToolsJson>();
            try{
                const auto old_ptr = std::dynamic_pointer_cast<VctToolsJson>(json_base_ptr);
    ...
    M_REGISTER_CLASS("vct-tools-json", VctToolsJsonTest)
